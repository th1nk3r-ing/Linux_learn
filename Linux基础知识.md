...
- 我们首先区分如下两个概念, 参数和标准输入:
  ...
  - 其实基本上 linux 的命令中很多的命令的设计是先从命令行参数中获取参数，然后从标准输入中读取，反映在程序上，命令行参数是通过 `main` 函数 `int main(int argc,char * argv[])` 的函数参数获得的，而标准输入则是通过标准输入函数例如 C 语言中的 `scanf` 读取到的。他们获取的地方是不一样的。
